...
import { Upload, X, FileDown, AlertCircle, Info } from 'lucide-react';
...
const COLUMN_HINTS: ColumnHint[] = [
  ...
  { column: 'unit_id', required: true, description: 'Volume unit id (e.g. litres, gallons)' },
...
export function FuelImportModal({ open, onOpenChange }: FuelImportModalProps) {
  ...
  async function handleImport() {
    ...
    const records = rows.map(coerceRow);
    try {
              ...
              {result.summary.failed > 0 && (
                <>
                    ...
                    <table className="min-w-full text-xs">
                      <thead className="bg-muted/50">
                        <tr>
                          ...
                          <th className="px-3 py-2 font-medium text-left text-muted-foreground">Error</th>
                        </tr>
                      </thead>
                      <tbody>
                  ...
                  <Button variant="ghost" size="sm" onClick={handleDownloadErrors}>
                    <FileDown className="h-3.5 w-3.5" /> Download error report
                  </Button>
                </>
              )}
            </div>
