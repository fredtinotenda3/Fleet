...
describe('TransportCostReportService.getDataQualityExceptions', () => {
  ...
  // Command Centre Slice A0: extractRawDisplayFields previously fell
  // through to Vansales's field names (rawRow.truck / rawRow.total) for
  // every non-'third-party' family, including 'swift' and 'depot-sto'.
  // Never exercised in production because Swift never posts and Depot
  // STO's stock-transfer category was out of this report's scope -- but
  // getDataQualityExceptions itself has no such scope restriction (it
  // reads whatever sheetFamily an exception was logged under), so these
  // two cases were always reachable once a Swift/Depot-STO batch had an
  // in-period posting to anchor on. Pinned here with the field names
  // read straight off SwiftImportRow / DepotStoImportRow.
  it('reads Swift raw fields correctly -- no registration/transporter column exists for Swift at all', async () => {
    const src1 = await seedSourceRecord({ sheetFamily: 'swift', sourceRowNumber: 1 });
    await seedPosting({ vehicleId: 'veh-1', sourceId: String(src1._id), amount: 100, reportingAmount: 100 });
    ...
    await seedException({
      ...
      sourceRowNumber: 2,
      column: 'consNumber',
      reason: 'Cons. Number is required',
      rawRow: { consDate: '06.01.26', consNumber: '', receiversName: 'ACME LTD', totalIncl: 642 },
    });
  ...
  it('reads Depot STO raw fields correctly, using its already-canonicalized DATE/registration/transporter/amount keys', async () => {
    const src1 = await seedSourceRecord({ sheetFamily: 'depot-sto', sourceRowNumber: 1 });
    await seedPosting({ vehicleId: 'veh-1', sourceId: String(src1._id), amount: 100, reportingAmount: 100 });
    ...
    await seedException({
      ...
      column: undefined,
      invalidValue: undefined,
      reason: 'Looks like a duplicate of an existing depot-sto record',
      rawRow: { date: '15.06.26', registration: 'AGL8230', transporter: 'SIGHTSCORE', amount: 350 },
    });
    ...
    expect(result.duplicates[0]).toMatchObject({
      rawDate: '15.06.26',
      rawRegistration: 'AGL8230',
      rawTransporter: 'SIGHTSCORE',
      amount: '350',
    });
  });
});
