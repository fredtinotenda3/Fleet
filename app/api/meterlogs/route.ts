...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch meter logs" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { _id: result.insertedId.toString(), ...insertData, date: insertData.date.toISOString() },
      { status: 201 }
    );
  } catch (error) {
    console.error("POST Error:", error);
    return NextResponse.json({ error: "Failed to create meter log" }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (result.matchedCount === 0) {
      return NextResponse.json({ error: "Meter log not found" }, { status: 404 });
    }

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("PUT Error:", error);
    return NextResponse.json({ error: "Failed to update meter log" }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to delete meter log" }, { status: 500 });
  }
}