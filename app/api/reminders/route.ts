...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (!body._id || typeof body._id !== "string") {
      return NextResponse.json({ error: "Missing or invalid reminder ID" }, { status: 400 });
    }
    ...
    if (body.license_plate) {
      ...
      if (!vehicleExists) {
        return NextResponse.json({ error: "New vehicle not found or deleted" }, { status: 400 });
      }
    }

    const dataToUpdate = { ...body, due_date: new Date(body.due_date) };
    ...
    if (!updatedDoc) {
      return NextResponse.json({ error: "Reminder not found after update" }, { status: 404 });
    }
