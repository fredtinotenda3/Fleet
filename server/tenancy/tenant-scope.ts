...
/**
 * True when a tenant-owned record's tenantId matches the caller's
 * scope tenantId.
 *
 * This is the single comparison every scoped service should use to
 * decide "is this record mine" -- exact string match, same as
 * BaseRepository.getTenantFilter() (`{ tenantId: scope.tenantId }`)
 * and the way resolveOrganization()/findBySlug() key their lookups.
 * Deliberately NOT case-insensitive or trimmed: tenantId values are
 * slugs generated by the system (see OrganizationService.generateSlug)
 * or literal ObjectId strings, never hand-typed by a caller, so a
 * mismatch in case or whitespace indicates a different record rather
 * than the same one written inconsistently.
 *
 * Does not itself special-case platform scope -- callers that need to
 * let a platform-wide caller through everything should check
 * `isPlatformScope(tenantId)` first, exactly as
 * `OrganizationService.getOrganization` does.
 */
