...
export class ExpenseRepository extends BaseRepository<Expense> {
  ...
  async findByLicensePlate(
    ...
    return this.findWithPagination(
      { license_plate: licensePlate.toUpperCase() } as Filter<Expense>,
      pagination,
      tenantId
    );
  }
  ...
  async getFilteredExpenses(
    ...
    const filter: Filter<Expense> = {} as Filter<Expense>;

    if (filters.license_plate) {
      (filter as any).license_plate = {
        $regex: filters.license_plate,
        $options: 'i',
      };
    }
    if (filters.type) {
      (filter as any).expense_type_id = new ObjectId(filters.type);
    }
    if (filters.startDate || filters.endDate) {
      (filter as any).date = {};
      if (filters.startDate) (filter as any).date.$gte = filters.startDate;
      if (filters.endDate) (filter as any).date.$lte = filters.endDate;
    }
    if (filters.minAmount !== undefined) {
      (filter as any).amount = { $gte: filters.minAmount };
    }
    if (filters.maxAmount !== undefined) {
      (filter as any).amount = {
        ...(filter as any).amount,
  ...
  async getExpenseStats(
    ...
    return {
      ...
      byType: Object.fromEntries(
        data.byType.map((t: any) => [t._id || 'Other', t.total])
      ),
      byMonth: Object.fromEntries(
        data.byMonth.map((m: any) => [m._id, m.total])
      ),
