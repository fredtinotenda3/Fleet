...
export class GetExpenseStatsHandler implements IQueryHandler<GetExpenseStatsQuery, ExpenseStats> {
  constructor(private readonly expenseRepo: ExpenseRepository) {}

  async execute(query: GetExpenseStatsQuery): Promise<ExpenseStats> {
    return this.expenseRepo.getExpenseStats(query.tenantId, query.dateRange, query.scope);
  }
}