...
export class FuelRepository extends BaseRepository<FuelLog> {
  ...
  async findByLicensePlate(licensePlate: string, tenantId: string, pagination: PaginationParams): Promise<PaginatedResponse<FuelLog>> {
    return this.findWithPagination({ license_plate: licensePlate.toUpperCase() }, pagination, tenantId);
  }

  async getFilteredLogs(filters: FuelFilters, tenantId: string, pagination: PaginationParams): Promise<PaginatedResponse<FuelLog>> {
    const filter: Filter<FuelLog> = {};
  ...
  async getFuelStats(tenantId: string, dateRange?: { startDate?: Date; endDate?: Date }): Promise<FuelStats> {
    ...
    return {
      ...
      logCount: data.count,
      efficiency: null, // Calculated in service layer
    };
  }

  async getMonthlyFuelConsumption(tenantId: string, months: number = 12): Promise<Array<{ month: string; fuel: number; cost: number }>> {
    const collection = await this.getCollection();
  ...
  async getTopFuelConsumers(tenantId: string, limit: number = 5): Promise<Array<{ license_plate: string; totalFuel: number; totalCost: number }>> {
    const collection = await this.getCollection();
    
    const pipeline = [
      { $match: this.getActiveFilter(tenantId) },
      {
        $group: {
          _id: '$license_plate',
          totalFuel: { $sum: '$fuel_volume' },
