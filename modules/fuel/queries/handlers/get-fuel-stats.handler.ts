...
export class GetFuelStatsHandler implements IQueryHandler<GetFuelStatsQuery, FuelStats> {
  constructor(private readonly fuelRepo: FuelRepository) {}

  async execute(query: GetFuelStatsQuery): Promise<FuelStats> {
    return this.fuelRepo.getFuelStats(query.tenantId, query.dateRange, query.scope);
  }
}