...
export class GetFuelEntryHeatmapHandler implements IQueryHandler<GetFuelEntryHeatmapQuery, FuelHeatmapCell[]> {
  constructor(private readonly fuelRepo: FuelRepository) {}

  async execute(query: GetFuelEntryHeatmapQuery): Promise<FuelHeatmapCell[]> {
    return this.fuelRepo.getFuelEntryHeatmap(query.tenantId, query.dateRange, query.scope);
  }
}