...
export class GetFuelByDriverHandler implements IQueryHandler<GetFuelByDriverQuery, DriverFuelConsumptionRow[]> {
  ...
  async execute(query: GetFuelByDriverQuery): Promise<DriverFuelConsumptionRow[]> {
    return this.fuelRepo.getFuelByDriver(query.tenantId, query.dateRange, query.limit, query.sortBy, query.scope);
  }
}